...
function create_notice_list(list) {
    ...
    noticeList.insertAdjacentHTML('beforeend',
        `<tr>
                ...
                <td class="tbl-content"><a href="/community/detail?no=${item.no}">
                    ${item.title}</a></td>
...
function create_page_num(pagination){
    ...
    pageNum.insertAdjacentHTML('beforeend',`
        <li onclick="find_all_notice(${prevBtnNum}, '', '', '', '공지사항')"><</li>              
    `)
    }
    for (let i = pagination.startPage; i <= pagination.endPage; i++){
        pageNum.insertAdjacentHTML('beforeend', `
        <li class="page-no" onclick="find_all_notice(${i}, '', '', '', '공지사항')">${i}</li>
        `)
    }

    if (pagination.existNextPage) {
        pageNum.insertAdjacentHTML('beforeend',`
        <li onclick="find_all_notice(${nextBtnNum}, '', '', '', '공지사항')">></li>            
    `)
    }
...
searchBtn.addEventListener('click', () => {
    // 검색한 키워드로 다시 DB정보 조회
    find_all_notice('1', '', '', keyword.value, '공지사항')
})
